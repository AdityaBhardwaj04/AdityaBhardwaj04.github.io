...
import { Folder, RefreshCw, Layers, Cpu, Terminal, ExternalLink, Star, ChevronDown } from 'lucide-react';
...
function renderArchitectureDiagram(projectId: string): React.ReactNode {
  switch (projectId) {
    case 'cipherchat':
      return (
...
function ProjectDetails({ project, demoInput, demoOutput, isDemoRunning, onRunDemo }: ProjectDetailsProps) {
  const diagram = renderArchitectureDiagram(project.id);
      ...
      {project.demoScenario && (
        <div className="border border-term-border rounded p-3 bg-term-bg/40 flex flex-col gap-2.5">
          ...
          <p className="text-xs text-term-lightgray italic">
            {project.demoScenario.prompt}
          </p>

          <div className="flex gap-2 flex-wrap">
            {project.demoScenario.inputs.map((inp, idx) => (
              <button
                key={idx}
                onClick={() => onRunDemo(inp)}
...
export default function ProjectsTab() {
  ...
  return (
    <>
      {/* MOBILE ACCORDION — visible below sm only */}
      <div className="sm:hidden flex flex-col gap-3">
        <div className="border border-term-border p-4 rounded bg-term-bg/50">
          <h2 className="text-term-lightgray mb-3 flex items-center gap-2 text-xs font-bold uppercase">
            <Folder size={14} /> SELECT PROJECT
          </h2>
          <div className="flex flex-col gap-2">
            {PROJECTS_DATA.map((project) => {
              const isSelected = project.id === mobileExpandedId;
              return (
                <div key={project.id}>
                  <button
                    onClick={() => handleMobileTap(project.id)}
                    className={`text-left p-3 w-full border transition-all text-xs flex flex-col gap-1 group ${
                      isSelected
                        ? 'border-term-green bg-term-darkgreen/40 text-term-green shadow-[0_0_5px_rgba(0,255,65,0.2)] rounded-t rounded-b-none border-b-0'
                        : 'border-term-border text-term-lightgray hover:border-term-green/50 hover:text-term-green bg-term-bg/10 rounded'
                    }`}
                  >
                    <div className="flex justify-between items-center w-full">
                      ...
                      <ChevronDown
                        ...
                        className={`flex-shrink-0 transition-transform duration-200 ${
                          isSelected ? 'rotate-180 text-term-green' : 'text-term-lightgray/50'
                        }`}
                      />
                    </div>
                  ...
                  <AnimatePresence initial={false}>
                    {isSelected && (
                      ...
                      >
                        <div className="border border-term-green border-t-0 rounded-b bg-term-darkgreen/10 p-3">
                          <ProjectDetails
                            project={project}
                            demoInput={demoInput}
      ...
      {/* DESKTOP TWO-COLUMN — visible at sm and above */}
        ...
        <div className="lg:col-span-4 flex flex-col gap-3">
          <div className="border border-term-border p-4 rounded bg-term-bg/50">
            <h2 className="text-term-lightgray mb-3 flex items-center gap-2">
              <Folder size={14} /> <span className="sm:inline hidden">[ </span>SELECT PROJECT<span className="sm:inline hidden"> ]</span>
            </h2>
            <div className="flex flex-col gap-2">
              {PROJECTS_DATA.map((project) => (
                <button
